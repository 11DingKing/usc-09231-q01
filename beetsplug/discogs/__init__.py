...
def normalize_media_format(media: str | None) -> str | None:
    """Return the canonical media type for a Discogs format name.

    Discogs describes digital releases as "Digital Media" or "WEB" while
    local libraries tag them as "File". Normalize these aliases so that
    search filters and parsed candidates agree on the media type before
    they reach the matcher. Unknown formats keep their original value.
    """
    if media is not None and media.casefold() in MEDIA_FORMAT_ALIASES:
        return "File"
...
class DiscogsPlugin(SearchApiMetadataSourcePlugin[IDResponse]):
    ...
    def get_search_query_with_filters(
        ...
        for tag, api_field in self.extra_discogs_field_by_tag.items():
            ...
            if tag == "media":
                values = (normalize_media_format(value) for value in values)
    ...
    def get_media_and_albumtype(
        ...
        if formats and (first_format := formats[0]):
            ...
            media = normalize_media_format(first_format["name"])
